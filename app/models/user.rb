  ...
  def self.import(file)
    ...
    CSV.foreach(file.path, encoding: 'UTF-8', headers: true, header_converters: :symbol, converters: :all) do |row|
      ...
      unless row[:email].nil? || row[:name].nil?
        ...
        unless row[:work_days].nil?
          agent.work_days = row[:work_days].split(',').map(&:to_i)
        end
        if agent.humanity_user_id == 0
          agent.humanity_user_id = HumanityAPI.set_humanity_id(row[:email], response)
        end
  ...
  def self.humanity_import
    ...
    humanity_users.each do |u|
      ...
      unless start_time.nil? || start_time == ""
        ...
        hour = time.hour
        start_time = if hour < 10
                       format('0%d:00', hour) + 'Z'
                     else
                       format('%d:00', hour) + 'Z'
                     end
      end
      ...
      shift_difference = (agent.end_time.to_i - agent.start_time.to_i).abs

      work_days = []
      work_days.push(0) if u['custom']['35708']['toggle'] == '1'
      work_days.push(1) if u['custom']['35711']['toggle'] == '1'
      work_days.push(2) if u['custom']['35712']['toggle'] == '1'
      work_days.push(3) if u['custom']['35713']['toggle'] == '1'
      work_days.push(4) if u['custom']['35714']['toggle'] == '1'
      work_days.push(5) if u['custom']['35715']['toggle'] == '1'
      work_days.push(6) if u['custom']['35716']['toggle'] == '1'
